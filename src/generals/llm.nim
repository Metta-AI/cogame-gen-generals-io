## Claude-backed command. A policy is just a prompt: the GAME server composes
## the seat's fogged view plus that seat's PLAYER_PROMPT and asks Claude what
## its army does for the next eight turns.
##
...
## hosted failures.
##
## The decision happens in the GAME pod, not the player container: the
## `anthropic_api_key` coworld secret is injected there
## (`game.runnable.env.ANTHROPIC_API_KEY_URI`), phase 60 greps the game log,
## and `docker_smoke.sh` forwards `ANTHROPIC_API_KEY` to the game container
## only.
##
...
type
  ...
  LlmClient* = ref object
    ...
    throttled*: bool
      ...
      ## rotate to. A retry inside the same turn cannot succeed, so the seat
      ## fails fast to the scripted fallback instead of spending the turn
      ## budget on a call that will be refused again.
...
proc resolveApiKey(): string =
  ...
  try:
    ...
    echo "gen-generals-io llm: failed to fetch ANTHROPIC_API_KEY_URI: ",
      error.msg
    result = ""
...
proc tryNextBedrockModel*(client: LlmClient, why: string): bool =
  ...
  echo "gen-generals-io llm: ", client.bedrockModels[client.bedrockModel - 1],
...
proc newLlmClient*(config: GameConfig): LlmClient =
  ...
  if bedrockEndpoint.len > 0 or bedrockToken.len > 0:
    ...
    echo "gen-generals-io llm: bedrock transport, model ",
  ...
  if result.apiKey.len > 0:
    ...
    result.curl = newCurly()
    echo "gen-generals-io llm: anthropic transport, model ", result.model
  else:
    result.transport = ltNone
    result.disabled = true
    ## The exact phrase phase 60 greps the GAME log for, alongside
    ## "falling back": "LLM provider is unavailable".
    echo "gen-generals-io llm: no credentials - the LLM provider is ",
      "unavailable; every directive turn is falling back to the scripted layer"
...
proc userMessage*(operatorPrompt: string, viewJson: string): string =
  ...
  ## fogged view, built server-side from `view(s)`.
