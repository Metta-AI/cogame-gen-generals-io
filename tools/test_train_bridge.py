"""Exercise complete GEN Generals.io games through the numeric bridge."""
    ...
    for policy in ("teacher", "random"):
        with subprocess.Popen(
            ...
            while observation["kind"] == "decision":
                ...
                assert {"terrain", "owner", "sight"} <= set(view["board"])
                assert observation["messages"][0]["content"].startswith("You command ONE ARMY")
                encoded = request({"kind": "encode"})
                assert encoded["decision_id"] == observation["decision_id"]
                assert len(encoded["values"]) == 834
                assert [len(head["choices"]) for head in encoded["action_heads"]] == [
                    6, 2, view["board"]["w"], view["board"]["h"], 8, 3, 4
                ]
                if policy == "teacher":
        ...
        print(f"{variant} {policy}: {decisions} decisions, 834 values")
